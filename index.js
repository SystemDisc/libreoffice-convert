...
const convertWithOptions = (document, format, filter, options, callback) => {
  ...
  const srcExt = (options || {}).srcExt || '';
  const ext = srcExt && `.${srcExt}`;
  ...
  return async.auto({
    ...
    saveSource: callback => fs.writeFile(path.join(tempDir.name, `source${ext}`), document, callback),
    convert: ['soffice', 'saveSource', (results, callback) => {
      ...
      args.push(path.join(tempDir.name, `source${ext}`));
